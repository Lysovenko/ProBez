...
int is_hole_vis (vector v2v, vector n, double R, double r)
{
...
int is_hole_cut (vector v2v, vector n, double R, double r)
{
  double sing, sinb, cosg, cosb, l, cosum;
...
P_MIRAGE proection_mirage(MIRAGE init_mir, SPHERE sph, int sphID, VIEWPOINT VP)
{
  P_MIRAGE res;

  tensor *tens = GetRequest(TENS);
  ...
  for(i = 0; i < res.nbeziers; i++)
  {
    ...
    {
      res.beziers[i].a = Projection(VP, VecSum(sph.o, init_mir.beziers[i].a));
      res.beziers[i].b = Projection(VP, VecSum(sph.o, init_mir.beziers[i].b));
      res.beziers[i].c = Projection(VP, VecSum(sph.o, init_mir.beziers[i].c));
      res.beziers[i].vis = 1;
    }
  }
  return res;
                           ...
                           MIRAGE * init_mirages)
{
  ...
  for (i = 0; i < ncor; i++)
  {
    ...
    mp1 = ProdScal (0.5, VecSum (polig[i], polig[(i > 0) ? i - 1 : ncor - 1]));
    mp2 = ProdScal (0.5, VecSum (polig[i], polig[(i < ncor - 1) ? i + 1 : 0]));
  ...
  for (i = 0; i < sph.nh; i++)
        ...
        (VecIneq (VP.vp, sph.o), sph.holes[i].n, sph.r, sph.holes[i].r))
    {
      vector *polig2;
      P_HOLE holelin;
      int j, cut = is_hole_cut (VecIneq (VP.vp, sph.o), sph.holes[i].n, sph.r,
                                sph.holes[i].r);
      //, draw_hole = 1;
...
holelin.figure=sph.holes[i].figure;
      ...
      for(j=0;j<ncor;j++)
         polig2[j+ncor]=ProdScal (0.5,
                        VecSum (polig2[j],
                                polig2[(j < (ncor - 1)) ? j + 1 : 0]));
      Allocator (res.holes =
                 realloc (res.holes,
                          sizeof (*res.holes) * (nholes + 1)));
      ...
      if (cut)
      {
        ...
        tpi =
          TriPlane (nv, DotProd (nv, O), sph.holes[i].n,
                    DotProd (sph.holes[i].n, sph.holes[i].o), nnp, DotProd (nnp,
                                                                            O),
                    &err);
        ...
        for (j = 0; j < ncor; j++)
        {
          ...
          mp1 =
            ProdScal (0.5,
                      VecSum (polig[j], polig[(j > 0) ? j - 1 : ncor - 1]));
          mp2 =
            ProdScal (0.5,
                      VecSum (polig[j], polig[(j < ncor - 1) ? j + 1 : 0]));

          case1 = 1;
          nroots =
            m_bez_plan_intersection (mp1, polig[j], mp2, O, pv1, &t1, &t2);
          if (nroots == 2 && t1 < 0.)
            t1 = t2;
          ...
          if (nroots && t1 > 0. && t1 < 1.)
          {
            p =
              VecSum (ProdScal ((1. - t1) * (1. - t1), mp1),
                      VecSum (ProdScal (2. * (1. - t1) * t1, polig[j]),
                              ProdScal (t1 * t1, mp2)));
          ...
          case2 = 1;
          nroots2 =
            m_bez_plan_intersection (mp1, polig[j], mp2, O, pv2, &t1, &t2);
          if (nroots2 == 2 && t1 < 0.)
            t1 = t2;
          ...
          {
          if(t_1<t_2)
          res.bs[j].be = add_be (res.bs[j].be, t_1, t_2, &res.bs[j].nbe);
          else
          res.bs[j].be = add_be (res.bs[j].be, t_2, t_1, &res.bs[j].nbe);
          }
          else
          {
            if(!case1) {
              ...
              {
                res.bs[j].be = add_be (res.bs[j].be, 0., t_1, &res.bs[j].nbe);
              }
              else
              {
                res.bs[j].be = add_be (res.bs[j].be, t_1, 1., &res.bs[j].nbe);
              }
            }
             ...
             if(!case2) {
              ...
              {
                res.bs[j].be = add_be (res.bs[j].be, 0., t_2, &res.bs[j].nbe);
              }
              else
              {
                res.bs[j].be = add_be (res.bs[j].be, t_2, 1., &res.bs[j].nbe);
              }
            }
            }
        ...
        pv1 = VecIneq (v2, ProdScal (DotProd (v1, v2) / VecAbs2 (v1), v1));
        pv2 = VecIneq (v1, ProdScal (DotProd (v1, v2) / VecAbs2 (v2), v2));
       if(DotProd(sph.holes[i].n,VecIneq(VP.vp,sph.holes[i].o))<0.)for (j = 0; j < ncor; j++)
          {
            vector mp1, mp2, p;
            int nroots, nroots2, case1, case2;
            double t1, t2;
            ...
            case1 = 1;
            nroots =
              m_bez_plan_intersection (mp1, polig2[j], mp2, sph.holes[i].o, pv1,
                                       &t1, &t2);
            if (nroots == 2 && t1 < 0.)
              t1 = t2;
            if (nroots && t1 > 0. && t1 < 1.)
...
// printf ("%g\n", t1);
              p =
                VecSum (ProdScal ((1. - t1) * (1. - t1), mp1),
                        VecSum (ProdScal (2. * (1. - t1) * t1, polig2[j]),
                                ProdScal (t1 * t1, mp2)));
              if (DotProd (pv2, VecIneq (p, sph.holes[i].o)) > 0.)
              {
                case1 = 0;
                if (DotProd (pv1, VecIneq (mp1, sph.holes[i].o)) <= 0.)
                {
                  holelin.bs[j].be =
            ...
            case2 = 1;
            nroots2 =
              m_bez_plan_intersection (mp1, polig2[j], mp2, sph.holes[i].o, pv2,
                                       &t1, &t2);
            if (nroots == 2 && t1 < 0.)
              t1 = t2;
            if (nroots2 && t1 > 0. && t1 < 1.)
            {
              ...
              {
                case2 = 0;
                if (DotProd (pv2, VecIneq (mp1, sph.holes[i].o)) <= 0.)
                {
                  holelin.bs[j].be =
                    add_be (holelin.bs[j].be, 0., t1,
                            &holelin.bs[j].nbe);
           ...
           if (case1 && case2
                && !(DotProd (pv2, VecIneq (mp1, sph.holes[i].o)) > 0.
                     && DotProd (pv1, VecIneq (mp1, sph.holes[i].o)) > 0.)
                && !(DotProd (pv2, VecIneq (mp2, sph.holes[i].o)) > 0.
                     && DotProd (pv1, VecIneq (mp2, sph.holes[i].o)) > 0.))
            {
              holelin.bs[j].vis = 0;
            }
          }
      ...
      if (sph.holes[i].figure == FIG_CYLINDER)
      {
        ...
        pvp =
          VecIneq (VP.vp,
                   ProdScal (DotProd
                             (sph.holes[i].n, VecIneq (VP.vp, sph.holes[i].o)),
                             sph.holes[i].n));
...
// printf ("%g\n", t1);
      ...
      //if (draw_hole)
        ...
        for (j = 0; j < ncor; j++)
        {
          ...
          nroots =
            m_bez_plan_intersection (mp1, polig2[j], mp2, O, nv, &t1, &t2);
          if (nroots == 2 && t1 < 0.)
          {
            t1 = t2;
            nroots = 1;
  ...
  if (sph.mir_id)
  {
    res.mir = proection_mirage (init_mirages[sph.mir_id - 1], sph, 0, VP);
    // fprintf(stderr,"id = %d nlines = %d
    // beziers\n",sph.mir_id,init_mirages[sph.mir_id-1].nlines,sph.mir_id,init_mirages[sph.mir_id-1].nbeziers);
  }
  return res;
}
