...
SPHERE parse_sphere (xmlDocPtr doc, xmlNodePtr cur)
{
  ...
  while (cur != NULL)
  {
    ...
    {
      key = xmlNodeListGetString (doc, cur->xmlChildrenNode, 1);
      if (!sscanf ((char*)key, "%lf %lf %lf", &res.o.x, &res.o.y, &res.o.z) == 3)
    ...
    if ((!xmlStrcmp (cur->name, (const xmlChar *) "hole")))
    {
      ...
      key = xmlNodeListGetString (doc, cur->xmlChildrenNode, 1);
      if (sscanf ((char*)key, "%lf %lf %lf %lf", &n.x, &n.y, &n.z, &r) == 4)
      {
        double r1 = sqrt (res.r * res.r - r * r);

        res.holes = realloc (res.holes, (res.nh + 1) * sizeof (*res.holes));
...
MIRAGE parse_mirage(xmlDocPtr doc, xmlNodePtr cur)
{
  ...
  while(cur != NULL)
  {
    if((!xmlStrcmp(cur->name, (const xmlChar *)"line")))
    {
      ...
      res.lines = realloc(res.lines, sizeof (*res.lines) * (res.nlines + 1));
...
KUPA3D parse_kupa3d (xmlDocPtr doc, xmlNodePtr cur)
{
  KUPA3D res;
  ...
  while (cur != NULL)
  {
    ...
    if ((!xmlStrcmp (cur->name, (const xmlChar *) "intersphcyl")))
    {
      ...
      {
        fprintf (stderr, __FILE__":%d: Spheres Overflow %d %d %d \n",__LINE__, isph.n1, isph.n2,
                 res.nsphers);
        continue;
      }
      res.cyls = realloc (res.cyls, sizeof (CYLINDER) * (res.ncyls + 1));
      res.cyls[res.ncyls] =
        interspher_cyl (res.sphers + isph.n1, res.sphers + isph.n2, isph.r,res.ncyls,isph.type);
      res.ncyls++;
    }
...
KUPAS3D interpret_kupas3d_xml (char *docname)
{
  ...
  while (cur != NULL)
  {
    if ((!xmlStrcmp (cur->name, (const xmlChar *) "box")))
    {
      ...
      res.kupas = realloc (res.kupas, sizeof (*res.kupas) * (res.nkupas + 1));
