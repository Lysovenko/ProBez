...
static gboolean expose_event (GtkWidget * da, GdkEventExpose * event,
                              gpointer * data)
{
  GdkGC *gc;
  GdkColor color;
  ...
  gdk_draw_rectangle (da->window, /* da->style->text_gc[1] */ gc, TRUE, 0, 0,
                      da->allocation.width, da->allocation.height);
  prb = GetRequest (PRIMITIVES);
  if (prb != NULL)
    prp_step_by_step_gdk (da->window, gc/*, ext_psc*/, prb);
  g_object_unref (gc);
  return TRUE;
}
...
static gint Delete (GtkWidget * widget, gpointer * data)
{
  // running=0;
  gtk_main_quit ();
  return FALSE;
...
void CreateDraw ()
{
  ...
  // //****************************************////
  {
    GtkWidget *FileMenu, *open_item, /* *open_xml_item, */ *file_item, /* *ViewSubmenu, */
    ...
    /* open_xml_item=gtk_menu_item_new_with_label("Open xml");
     * g_signal_connect (GTK_OBJECT(open_xml_item), "activate",
     * G_CALLBACK (load3d_xml_file), window);
     * gtk_menu_shell_append(GTK_MENU_SHELL(FileMenu),open_xml_item); */
  ...
  g_signal_connect (GTK_OBJECT (window), "key-press-event",     // "key-press-event"
  ...
  SetRequest (KUPA_3D, calloc (1, sizeof (KUPA3D)));
